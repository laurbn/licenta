...
public class OrTranslator implements Translator {
    ...
    public boolean isApplicableToVPlus(String instruction) {
        return instruction.contains(" OR ");
    }
    ...
    public void translateVPlus(List<String> instructions, Integer index) {
        Pattern p = Pattern.compile("\\w+(?=\\s+OR)");
        ...
        instructions.add(index, "value := BitOr("+testValue1+", "+testValue2+");");
        if (hasBeenCalled()==false) {  //come back and add AndTranslator redundancyEliminator
